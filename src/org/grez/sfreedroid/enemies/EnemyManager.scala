...
class EnemyEntry(var mapPosX: Float, var mapPosY: Float, var a: Int, val enemyType: Int = 1, var health: Int = 5 ){
  override def toString = "[x ="+mapPosX+"; y= "+ mapPosY+"; a="+a+"]"
}

object EnemyEntry {
  def apply (mapPosX: Float, mapPosY: Float, a: Int) = new EnemyEntry(mapPosX,mapPosY, a);
}
...
object EnemyManager {
  ...
  var allEnemies: List[EnemyEntry] = List(EnemyEntry(0f,3f, 0), EnemyEntry(1f,4f, 1),EnemyEntry(1f,5f, 2),
    ...
    )::: List.fill(9)(new EnemyEntry(Random.nextInt(MapDefaults.SIZE_X),Random.nextInt(MapDefaults.SIZE_Y),Random.nextInt(8)));
  ...
  val draw139: (Float,Float, Int) => Unit = if (robot139.isLeft) ((x,y, angle) => simpleDrawOnScreen(x,y)) else ((x,y, angle)=> {
    ...
    r.texture.draw(35 + x, y- r.spec.origWidth/2 );
  }) ;
        ...
        for (enemy <- allEnemies) {
          ...
          val (curX, curY) = {
            if (isOdd(mapPosY.toInt))
              (DEF_WIDTH * mapPosX, DEF_HEIGHT / 2 * (mapPosY - 1))
            else
              ((DEF_WIDTH * mapPosX) - (DEF_WIDTH / 2), (DEF_HEIGHT / 2 * mapPosY) - (DEF_HEIGHT / 2));
          }
